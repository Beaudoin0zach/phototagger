...
-- Atomically ensures each of NEW_KEYWORDS is present on the media item
-- identified by ITEM_ID, addressed directly by id (no positional index, so
-- this is unaffected by the library growing or shrinking between calls).
-- Reads the current keyword list, adds anything missing (case-insensitive
-- compare), writes back once, and returns the before/after lists so the
-- caller can journal the true pre-write state instead of a stale snapshot.
on run argv
    if (count of argv) is not 2 then error "usage: sync_library_keywords_by_id.applescript ITEM_ID NEW_KEYWORDS"
    set itemId to item 1 of argv
    set newKeywordsText to item 2 of argv
